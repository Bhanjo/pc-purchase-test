...
const characterInner = document.getElementById("characterInner");
function resultText() {
    let myPurchase;
    let purchaseDetail;
    let characterImg;
    switch (true) {
        ...
        case (PerPripoint <= -10):
            ...
            characterImg = `<img class="characterSize" src="img/character_2.png">`;
            break;
        case (PerPripoint <= 10):
        ...
        case (PerPripoint <= 30):
            ...
            단 1만원 차이라도 용서못하며 싸게사기 위해 며칠이고 기다릴 인내가 있습니다
            이를 위해 당X마켓이나 다X와에 폭풍 검색을 하기도 하네요
            하지만 이때문에 퀄리티가 낮은 제품을 골라 낭패를 보지 않게 주의가 필요합니다!`;
            characterImg = `<img class="characterSize" src="img/character_4.png">`;
            break;
        case (PerPripoint <= 50):
            ...
            characterImg = `<img class="characterSize" src="img/character_5.png">`;
            break;
        default:
    ...
    const mytestText = `<h3>당신의 성향은?</h3>${characterImg}<br><a>${myPurchase}</a>`;
